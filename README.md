...
초대 링크로 두 플레이어가 대국하는 Vercel 배포형 Next.js 체스 게임입니다. 모든 착수, 턴, 체크메이트, 무승부, 기권, 타이머 판정은 서버에서 `chess.js`로 검증합니다. 한 방에서 여러 라운드를 이어서 진행할 수 있고, 라운드 승수와 캡처 점수가 누적됩니다.
...
- 기물 캡처 점수는 폰 1점, 나이트/비숍 3점, 룩 5점, 퀸 9점으로 계산합니다. 킹은 점수가 없으며, 킹을 잡는 대신 체크메이트로 라운드가 종료됩니다.
- 제한시간이 끝나면 현재 라운드의 캡처 점수가 높은 플레이어가 승리하고, 동점이면 무승부입니다.
- 종료 후 `같은 방에서 재대국`으로 방 링크를 유지한 채 다음 라운드를 시작할 수 있습니다.
